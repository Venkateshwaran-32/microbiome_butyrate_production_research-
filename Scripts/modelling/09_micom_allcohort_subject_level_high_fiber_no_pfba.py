...
SUMMARY_OUTPUT = Path("Results/subject_level_fba/tables/09_allcohort_subject_community_growth_high_fiber_no_pfba.csv")
TAXON_OUTPUT = Path("Results/subject_level_fba/tables/09_allcohort_subject_taxon_growth_high_fiber_no_pfba.csv")
FLUX_OUTPUT = Path("Results/subject_level_fba/tables/09_allcohort_subject_reaction_flux_nonzero_long_high_fiber_no_pfba.csv")
BUILD_REPORT = Path("Results/subject_level_fba/reports/09_allcohort_subject_level_high_fiber_no_pfba_build_report.txt")
...
CSV_OUTPUT_SPECS = [
    csv_output_spec(
        SUMMARY_OUTPUT,
        "one row per subject_id for the high_fiber diet, MICOM cooperative tradeoff without pFBA",
        [
            ...
            col(
                "objective_value",
                "Objective value returned by MICOM for this subject-level cooperative tradeoff solve without pFBA.",
                "objective_value = optimization objective reported by MICOM after cooperative_tradeoff with pfba=False",
            ),
            ...
            col("pfba", "Boolean flag showing whether pFBA flux extraction was requested; always False in this script."),
    ...
    csv_output_spec(
        TAXON_OUTPUT,
        "one row per subject_id x taxon_id for the high_fiber diet without pFBA",
        [
            col("subject_id", "Subject identifier from the all-cohort subject-level MICOM input table."),
            col("cohort", "Cohort label carried through from the subject metadata."),
            col("age_years", "Subject age in years from the metadata table."),
    ...
    csv_output_spec(
        FLUX_OUTPUT,
        "one row per subject_id x compartment x reaction_id with nonzero exported flux for optimal solves (no pFBA)",
        [
            col("subject_id", "Subject identifier from the all-cohort subject-level MICOM input table."),
            col("cohort", "Cohort label carried through from the subject metadata."),
            col("age_years", "Subject age in years from the metadata table."),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run all-cohort subject-level MICOM with high-fiber diet and flux export, without pFBA."
    )
    parser.add_argument(
        "--subject-id",
        action="append",
        dest="subject_ids",
        help="Subject ID to include. Repeat the flag to include multiple subjects.",
    )
...
def select_subject_ids(qc_rows: list[dict[str, str]], args: argparse.Namespace) -> list[str]:
    ...
    if args.subject_ids:
        requested = set(args.subject_ids)
        selected_rows = [row for row in selected_rows if row["subject_id"] in requested]
        missing = requested - {row["subject_id"] for row in selected_rows}
        if missing:
            raise ValueError(f"No included subjects matched --subject-id values: {sorted(missing)}")
...
def main() -> None:
    ...
    try:
        ...
        raise ImportError(
            ...
            "`pip install micom`, then rerun Scripts/modelling/09_micom_allcohort_subject_level_high_fiber_no_pfba.py."
        ) from exc
    ...
    report_lines = [
        "All-cohort subject-level MICOM high-fiber no-pFBA build report",
        f"Subject input: {SUBJECT_INPUT}",
        f"QC input: {QC_INPUT}",
        f"Diet file: {DIET_CSV}",
        f"Tradeoff fraction: {TRADEOFF_FRACTION}",
        f"Selection filters: subject_id={','.join(args.subject_ids) if args.subject_ids else 'ALL'}, cohort={args.cohort or 'ALL'}, age_group={args.age_group or 'ALL'}, limit={args.limit or 'ALL'}",
        f"Flux export enabled: True",
        f"pFBA enabled: False",
        "",
